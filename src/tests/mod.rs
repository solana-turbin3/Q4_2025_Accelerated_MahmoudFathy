...
mod tests {
    ...
    const PROGRAM_ID: &str = "i7wWmYFi9LXizzU8p7b2iTm63qMftM4ykXoeqSxRKeV";
    ...
    fn send_singed_tx(svm: &mut LiteSVM, ix: Instruction, payer: &Keypair) -> TransactionResult {
    ...
    fn setup() -> (
        ...
        svm.airdrop(&owner.pubkey(), 10 * LAMPORTS_PER_SOL)
            .expect("Airdrop failed");
        svm.airdrop(&mint_authority.pubkey(), 100 * LAMPORTS_PER_SOL)
            .expect("Airdrop failed");

        // Load program SO file
        msg!("[SETUP] The path is!! {}", env!("CARGO_MANIFEST_DIR"));
        let so_path = PathBuf::from("./target/sbpf-solana-solana/release/fundraiser.so");
    ...
    pub fn test_initialize_instruction() {
        let (mut svm, maker, _, _, mint, _, fundraiser, vault) = setup();
        ...
        let tx = send_singed_tx(&mut svm, init_ix, &maker)
            .map_err(|e| {
                format!(
                    "[test_initialize_instruction] Initialize Transaction Failed: {:?}",
                    e
                )
            })
            .unwrap();
        ...
        msg!(
            "[test_initialize_instruction] CUs Consumed: {}",
            tx.compute_units_consumed
        );

        // POSTCOND
        let fundraiser_pda_state = svm
            .get_account(&fundraiser)
            .ok_or("[test_initialize_instruction] Could not retrieve fundraiser PDA")
            .unwrap();

        let s_fundraiser =
            bytemuck::try_from_bytes::<crate::state::Fundraiser>(&fundraiser_pda_state.data)
                .unwrap();
    ...
    fn test_contribute_instruction() {
        let (mut svm, maker, owner, mint_authority, mint, maker_ata, fundraiser, vault) = setup();
        ...
        msg!(
            "[test_contribute_instruction] Contributor PDA: {}\n",
            &contributor_pda
        );
        ...
        let tx = send_singed_tx(&mut svm, contribute_ix, &user)
            .map_err(|e| {
                format!(
                    "[test_contribute_instruction] Contribute Transaction Failed: {:?}",
                    e
                )
            })
            .unwrap();
    ...
    fn test_refund_instruction() {
        ...
        let duration: u64 = 2 * 7 * 24 * 60 * 60; // 2 weeks
        {
            ...
            send_singed_tx(&mut svm, init_ix, &maker)
                .map_err(|e| {
                    format!(
                        "[test_refund_instruction] Initialize Transaction Failed: {:?}",
                        e
                    )
                })
                .unwrap();
...

        {
            ...
            send_singed_tx(&mut svm, contribute_ix, &user)
                .map_err(|e| {
                    format!(
                        "[test_refund_instruction] refund Transaction Failed: {:?}",
                        e
                    )
                })
                .unwrap();
        ...
        // time travel
        let mut now = svm.get_sysvar::<Clock>();
        let now_unix = now.unix_timestamp;

        now.unix_timestamp = now_unix + duration as i64 + 1; //
        svm.set_sysvar::<Clock>(&now);

        let refund_data_ix = [
            vec![crate::instructions::FundraiserInstructions::Refund as u8],
            10_000_000u64.to_le_bytes().to_vec(), // Discriminator for "Refund" instruction
        ]
        .concat();
        ...
        let tx = send_singed_tx(&mut svm, refund_ix, &user)
            .map_err(|e| {
                format!(
                    "[test_refund_instruction] Refund Transaction Failed: {:?}",
                    e
